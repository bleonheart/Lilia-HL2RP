...
function ENT:ejectPilot(no)
    ...
    pilot:UnSpectate()
    if not self.noRespawn and pilot:Alive() then pilot:Spawn() end
    pilot:SetPos(self.spawn or self:GetPos())
    pilot:DrawViewModel(true)
    pilot:CrosshairEnable()
    if self.health <= 0 then pilot:KillSilent() end
    self:SetPilot(NULL)
end
...
function ENT:createFlashSprite()
    ...
    self.flashSprite:SetAttachment(self, self:LookupAttachment(SCANNER_ATTACHMENT_LIGHT))
...
function ENT:Initialize()
    ...
    self:PrecacheGibs()
    if SERVER then self:createFlashSprite() end
...
function ENT:emitDelayedSound(source, delay, volume, pitch)
    timer.Simple(delay or 0, function() if IsValid(self) then self:EmitSound(source, volume, pitch) end end)
end
...
function ENT:handlePilotMove()
    ...
    if pilot:GetMoveType() ~= MOVETYPE_NONE then pilot:SetMoveType(MOVETYPE_NONE) end
end
...
function ENT:discourageHitGround()
    ...
    if trace.Hit then self.targetDir.z = self.minHoverPush.z end
end
...
function ENT:Think()
    ...
    if IsValid(self:GetPilot()) then self:handlePilotMove() end
...
function ENT:updateDirection()
    if IsValid(self:GetPilot()) then self:facePilotDirection() end
end
...
function ENT:OnTakeDamage(dmgInfo)
    ...
    if self.health <= 0 then
        ...
        local pilot = self:GetPilot()
        if IsValid(pilot) then pilot:SetHealth(self.health) end
        self:doDamageSound()
    end
end
