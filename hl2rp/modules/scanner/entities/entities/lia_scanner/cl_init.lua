include("shared.lua")
local knots = {Vector(-20, 0, 0), Vector(-30, 0, 0), Vector(120, 0, 0), Vector(90, 0, 0),}
function ENT:Think()
    ...
    if self.wheel < 0 then self.wheel = 360 end
    ...
    self:playFlySound()
    if self.sound then self.sound:ChangePitch(math.min(80 + (lengthSqr / 10000) * 20, 255), 0.5) end
end

function ENT:playFlySound()
    if not self.sound then
        local source = "npc/scanner/cbot_fly_loop.wav"
        if self:GetModel():find("shield_scanner") then source = "npc/scanner/combat_scan_loop6.wav" end
